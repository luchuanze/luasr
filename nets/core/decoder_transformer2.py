...
from nets.core.attention import MultiHeadAttention
...
class Transformer(torch.nn.Module):
    def __init__(self,
        ...
        if self.normalize_before:
            after_norm = torch.nn.LayerNorm(attention_dim, eps=1e-5)
        else:
            after_norm = None
...
class TransformerLayer(torch.nn.Module):
    def __init__(self,
        ...
        self.src_attention = MultiHeadAttention(
            attention_dim,
            nhead=nhead,
            dropout_rate=0.0
        )
    ...
    def forward(self,
        ...
        x2 = self.self_attention(tgt_q, tgt, tgt, key_padding_mask=tgt_q_mask)
        ...
        x3 = self.src_attention(x, memory, memory, key_padding_mask=memory_key_padding_mask)
